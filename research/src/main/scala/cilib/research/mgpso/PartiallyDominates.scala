...
object PartiallyDominates { // no longer check for 3 obj since the way the random indices are stored caters for all number of obj already
	...
	def apply(envX: Benchmark)(particleA: MGParticle, particleB: MGParticle, i123: (Int, Int, Int)): Boolean =
		if (particleA.pos.fitness == particleB.pos.fitness){
			...
			val partialParticleA = List[Double](particleA.pos.fitness.toList(i123._1), particleA.pos.fitness.toList(i123._2), particleA.pos.fitness.toList(i123._3))
			val partialParticleB = List[Double](particleA.pos.fitness.toList(i123._1), particleA.pos.fitness.toList(i123._2), particleA.pos.fitness.toList(i123._3))
