...
object KneePoint {
  def apply(archiveCollection: NonEmptyList[MGParticle],
    ...
    if (kp_cond0.isDefined) return kp_cond0
    ...
    val kp_cond1: Option[MGParticle] =
      if (N1ds.head == N1max && N2ds.head != N2max) Some(archiveCollection.toList(centres.head._2))
      else if (N1ds.head != N1max && N2ds.head == N2max)
        Some(archiveCollection.toList(centres(1)._2))
      else None
    if (kp_cond1.isDefined) return kp_cond1
    ...
    if (kp_cond2.isDefined) return kp_cond2
  ...
  def kneePoint(collection: NonEmptyList[MGParticle], R: NonEmptyList[Double]): Option[MGParticle] =
    apply(collection, R)
}
